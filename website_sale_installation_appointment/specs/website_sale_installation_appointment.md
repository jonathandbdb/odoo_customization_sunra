...
| D59 | ¿Como se ordena el formulario del link? | **En cuatro secciones numeradas**, con titulo y una linea de ayuda: **1 Tus datos** (nombre, correo y la **pregunta principal de telefono**), **2 Sobre tu puerta** (las demas preguntas del tipo de cita), **3 Direccion de instalacion** (el bloque de D58) y **4 Fotos del lugar**. En ese formulario la **etiqueta va arriba del campo** y a lo ancho: el nativo la pone en `col-sm-3` contra un `col-sm-9`, y con seis campos seguidos se lee como un muro. **Solo el camino del link**: el del checkout conserva el layout nativo (son dos campos). El gate es `appointment.type._is_installation_link_form()`, que reusa `installation_fsm_project_id` (el marcador de "agendado fuera del eCommerce", D8) y excluye el checkout. La clase marcadora del form va por `t-attf-class` **repitiendo la del core**: el nativo la declara estatica y en el render el atributo dinamico pisa al estatico (`ir_qweb.py:1946`). En el link, las etiquetas propias de los bloques de direccion y fotos (con su " *") van ocultas (`d-none`): las reemplaza el titulo de la seccion. **Telefono en la seccion 1**: la pregunta principal de telefono es la primera de tipo *Telefono* del tipo de cita (`appointment.type._get_main_phone_question()`, la misma cuya respuesta el core vuelca en `phone` del contacto), asi que es un dato de contacto: entre las preguntas de la puerta se lee fuera de lugar. En el link se pinta debajo del correo y se saltea en la seccion 2; las demas preguntas de tipo *Telefono* (si las hubiera) quedan en la seccion 2. Sin pregunta de telefono en el tipo de cita, la seccion 1 queda con nombre y correo. La seccion 2 solo se titula si le queda alguna pregunta. El camino del checkout no cambia. |
...
- Formulario de la cita en 4 secciones numeradas (D59), con el telefono de contacto junto a nombre y correo, con bloque de **direccion de instalacion** (D58) y de fotos, y **aviso visible de obligatoriedad** en ambos (D61).
...
- `appointment_form` (hereda `appointment.appointment_form`):
  ...
  - **camino del link** (D59): clase `o_installation_sections` en el form (por `t-attf-class`,
    repitiendo la del core) y las 4 secciones numeradas (`installation_form_section`). El
    encabezado de la seccion 2 se pinta solo si quedan preguntas fuera de la principal de telefono
    (`appointment_type.question_ids - main_phone_question`);
  - **telefono en "Tus datos"** (D59), sin duplicar el input:
    - `installation_phone_input` — template nuevo con el `<input>` de las preguntas de telefono
      (`question._answer_input_attrs()`, `name="question_<id>"`, valor precargado desde
      `partner_data['phone']` si es la principal, `required`, `placeholder` y
      `data-is-main-phone-question`). El `//input[@type='phone']` con `position="replace"` pasa a
      ser un `t-call` a este template: `question` e `isMainPhoneQuestion` llegan del bucle del core;
    - `installation_main_phone_row` — template nuevo con la fila (`div.row.mb-4`, `label.col-sm-3
      .col-form-label.fw-normal` con `for="question_<id>"` y el mismo texto que el core, nombre +
      " *" si es obligatoria, y `div.col-sm-9` con el `t-call` a `installation_phone_input`). Se
      llama dentro del `replace` de `//label[@for='email']/..`, **despues** del `t-if`/`t-else`, con
      `t-if="installation_link_form and main_phone_question"` y en el cuerpo del `t-call`
      `question = main_phone_question` e `isMainPhoneQuestion = True` (el `t-set` dentro del
      `t-call` no se filtra al resto del form);
    - el `div.row` del bucle del core (`//t[@t-foreach='appointment_type.question_ids']/div[hasclass('row')]`)
      recibe por `position="attributes"` el `t-if` = `question.question_type != 'text' and not
      (installation_link_form and question == main_phone_question)`: conserva la condicion
      nativa y saltea la principal de telefono solo en el link. El `position="before"` de la guia
      de medidas usa el mismo locator y no se ve afectado;
    - el input conserva `name` y `data-is-main-phone-question="True"`: el submit nativo y la
      interaccion del form (precarga desde `localStorage`) no dependen de su posicion;
...
36. **RB36**: En el formulario del **link**, la pregunta principal de telefono del tipo de cita se
    pide en "Tus datos", debajo del correo, y no se repite en "Sobre tu puerta". Si es obligatoria,
...
- **Requisito de configuracion de las preguntas del link**: el tipo de cita del **link** lleva la
  pregunta **"Telefono de contacto"** (`question_type = phone`, `answer_format = phone`,
  obligatoria), la misma que usa el tipo del checkout (las preguntas son reutilizables). Sin ella el
  formulario del link no pide telefono: la seccion 1 queda con nombre y correo y el contacto se crea
  sin `phone`.
- **Tipo de cita del link cuya unica pregunta es el telefono**: el telefono va a "Tus datos" y la
  seccion 2 no se titula (no le queda ningun campo).
- **Varias preguntas de tipo Telefono en el link**: solo la primera (`_get_main_phone_question()`)
  sube a "Tus datos" y alimenta `phone` del contacto; las demas quedan en "Sobre tu puerta".
...
- [x] **CA53**: El formulario del **link** se ve en 4 secciones numeradas (Tus datos · Sobre tu puerta · Direccion de instalacion · Fotos del lugar), con la etiqueta arriba del campo. "Tus datos" muestra nombre, correo y, debajo del correo, el **telefono de contacto** (la pregunta principal de telefono del tipo de cita), que no aparece en "Sobre tu puerta".
- [x] **CA54**: El formulario del **checkout** conserva el layout nativo: sin secciones, sin bloque de direccion y sin input de fotos.
- [x] **CA63**: En el formulario del **link** con la pregunta obligatoria "Telefono de contacto": enviarlo vacio lo frena el navegador en el campo; con un valor sin formato de telefono (menos de 8 digitos, forzado saltando la validacion del navegador) el servidor vuelve al formulario con el detalle; con `11 5555-1234` la cita se crea y el contacto que reserva queda con ese telefono en `phone`. El formulario del **checkout** no cambia: el telefono sigue en su lugar nativo, entre las preguntas.
...
| Pregunta principal de telefono en el contexto del form | `enterprise/appointment/controllers/appointment.py:L584` | `'main_phone_question': appointment_type._get_main_phone_question()` → disponible en `appointment_form` (D59) |
| Que es la pregunta principal de telefono | `enterprise/appointment/models/appointment_type.py:L623-625` | La primera de `question_ids` con `question_type == 'phone'` |
| Bucle de preguntas y fila nativa | `enterprise/appointment/views/appointment_templates_registration.xml:L53-64` | `div.row` con `t-if="question.question_type!='text'"` y el `input type="phone"` con `data-is-main-phone-question` |
| La respuesta principal va al `phone` del contacto | `enterprise/appointment/controllers/appointment.py:L743-751` y `:L816-821` | El submit toma la respuesta de `main_phone_question` como `phone` del contacto nuevo o del existente sin telefono |
| La interaccion ubica el telefono por atributo | `enterprise/appointment/static/src/interactions/appointment_form.js:L51` | `querySelector('input[data-is-main-phone-question="True"]')`: mover la fila no la rompe |
