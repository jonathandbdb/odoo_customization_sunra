...
| **Estado** | `verified` (1.0.2 quedo `verified`; 1.1.0 implementado y pendiente de pasada de @reviewer; 1.2.0 = rediseno del bloque de precio, clarificado con la cliente el 11-09-2026, **implementado** el 11-09-2026: T01..T10 ejecutadas, DOM verificado por `odoo shell` sobre la copia COW del sitio (`website_id=1`, `lang=es_AR`) y sobre las vistas genericas; revisado por @reviewer —**sin criticos**, 3 IMPORTANTES y varios MENORES— y verificado visualmente en el navegador por el orquestador (ficha, grilla, carrito, cambio de variante, estado sin reglas); ronda de correcciones post-review cerrada el 11-09-2026 (I1..I3, M1..M9, ver Notas de implementacion); **queda `implemented`**, pendiente de la re-verificacion final del orquestador para pasar a `verified`) |
...
Devuelve `[{name, label, price, price_formatted, price_type, badge}]`, en el orden de `sequence` de las
...
tenga ya una regla aplicada. `price_type` viaja tambien aca —la firma es **la misma** que la del helper del
producto— para que la fila del carrito decida pill y destacado sin volver a resolver la regla (D23).
...
| **CA20** | El precio sin impuestos nacionales de `l10n_ar_website_sale` se sigue publicando, **arriba** del recuadro, en ficha y grilla, y se sigue repintando al cambiar de variante | OK |
...
- `README.md` del modulo — **a actualizar**: la seccion "La vidriera" describe hoy "un segundo precio
  debajo del precio"; pasa a describir el bloque de D17 (orden, pill, etiqueta) y el **contrato del
  contenedor** `div[@name='wspmp_box']` / `div[@name='wspmp_installments']` (locator por `name`) con el
  puente de cuotas (D21).
- `__manifest__.py` — **a actualizar**: el `summary` (`__manifest__.py:5`, "con segundo precio en la
  vidriera") y el `description` (`__manifest__.py:13`, «aparece "$X con TRANSFERENCIA" debajo del
  precio») describen el bloque de 1.1.0 y quedan **falsos** en 1.2.0 —ademas el string
  `with %(method)s` se da de baja en T08—: pasan a describir el recuadro (precio de lista tachado,
  pill del porcentaje, precio del medio protagonista y etiqueta `pagando con <medio>`). Va junto con
  el bump de `version` a 1.2.0, en T10.
- `static/description/index.html` — **a actualizar**: mismo cambio de funcionalidad visible.
- `README.md` raiz de `odoo_customization_sunra` — la fila del modulo ya existe; se toca solo si cambia
  el resumen de una linea.
- `README.md` de `website_sale_installment_plans_ux` — lo actualiza **su** tarea (el puente), no esta.

## Plan del cambio en curso

> Rediseño del bloque de precio (1.1.0 -> **1.2.0**). El plan anterior (T01..T18, alta del modulo y
> fixes hasta 1.1.0) esta cerrado y no se repite aca.

| Tarea | Descripcion | Depende de | Archivos | Cubre | Estado |
|-------|-------------|-----------|----------|-------|--------|
| **T01** | Datos del bloque: `_get_percentage_label()` en la regla, y `label` (prefijo) + `name` + `badge` + `price_type` en los vals de producto y de pedido; sale el string `with %(method)s` | — | `models/payment_method_website_price.py`, `models/product_template.py`, `models/sale_order.py` | CA23 | hecho |
| **T02** | Ficha (`priority="18"`), en **dos operaciones**: (a) `xpath //div[@name='product_price']` + `inside` arma `div[@name='wspmp_box']` (recuadro **ultimo hijo**, D18) con el pill, las filas de precio con su etiqueta y el slot vacio `div[@name='wspmp_installments']`; (b) `xpath //div[@name='wspmp_installments']` + `before` con el `move` de `//div[@name='product_price_container']` como **hijo directo y vacio**. Locators **por `name`** (D21) y la clase `o_wspmp_box` repetida dentro del `t-attf-class`. **Reemplaza** el ancla `after` de 1.1.0. La clase `o_wspmp_discounted` (ausente con recargo) es la que apaga el tachado | T01 | `views/website_sale_templates.xml` | CA07, CA19, CA20, CA21, CA23 | hecho (ronda de fixes 11-09-2026 tras verificacion visual: el `move` paso de `inside` de `wspmp_box` a `before` de `wspmp_installments`, para que el orden en el DOM no dependa de `order: -1`, ver Notas de implementacion) |
| **T03** | Grilla: el mismo recuadro en la tarjeta, en **tres operaciones** — (a) `xpath //div[hasclass('product_price')]` + `inside` (aca `hasclass` si vale: clase estatica del core) arma `div[@name='wspmp_box']` con pill, filas y slot; (b) `move` de `span[@aria-label='Sale price']`; (c) `move` de `t[name='product_base_price']`, cada uno hijo directo y vacio de su propio `xpath //div[@name='wspmp_installments']` + `before`. El recuadro **no** hereda layout: lo fija el CSS de T05 (`--o-wsale-card-price-display` no tiene fallback). Misma clase condicional `o_wspmp_discounted` que en la ficha | T01, T02 | `views/website_sale_templates.xml` | CA06, CA19, CA20, CA21, CA23 | hecho (locator de `span[@aria-label='Sale price']` cambiado a `hasclass('fw-bold')`; y ancla `before` de `wspmp_installments` en vez de `inside` de `wspmp_box`, mismo motivo que T02; ver Referencias al core y Notas de implementacion) |
| **T04** | Carrito y checkout: la fila por medio adopta la etiqueta nueva y **su propio pill** (una fila por regla, cada una con su porcentaje; sin recuadro ni slot, D23) | T01 | `views/website_sale_templates.xml` | CA09 | hecho |
| **T05** | SCSS del bloque: recuadro (`#A3EA24` hardcodeado, D17), pill de marca, precio protagonista, tachado del precio del core **solo bajo `o_wspmp_discounted`** (con recargo no hay tachado), etiqueta sin mayusculas y sin corte a 360 px, breakpoint `md`. En la **grilla** el recuadro fija su propio layout (`display: flex` + `width`/`flex-basis` 100 %) para no depender de `--o-wsale-card-price-display`; en la **ficha** normaliza el `justify-content-end` y el ancho que trae el contenedor movido | T02, T03, T04 | `static/src/scss/payment_method_price.scss` | CA18, CA23 | hecho, con **dos rondas**. 1ra: `display: flex` en vez de `display: block` (el `move` inicial agregaba el precio de lista **al final** del recuadro y hacia falta `order: -1` en flex/grid para reordenarlo; `display: block` no soporta `order`). 2da (11-09-2026, tras verificacion visual): (a) selectores **nunca por `aria-label`** (atributo traducible, sale `"Precio de venta"` en es_AR y el selector con el string en ingles no matchea) — se usa `span.fw-bold`/`del` hijo directo; (b) `justify-content: flex-start !important` en la ficha (el core trae `justify-content-end` **tambien** `!important`, hacia falta el propio); (c) el `<small>` de la AR (D18) se estiliza chico/gris por CSS (`[name='product_price'] > small`); (d) el `move` paso de `inside` de `wspmp_box` a `before` de `wspmp_installments` (T02/T03), asi el orden en el DOM no depende de `order`, que no aplica con el recuadro en `display: contents` (CA19). Detalle en Notas de implementacion |
| **T06** | SCSS de la grilla: tarjetas de igual altura con el bloque precio+CTA al pie. **Primero verificar en el navegador** si el tema deja `.o_wsale_product_sub` en `flex` o en `display: contents` (el core avisa que puede caer en `contents`) y **anotarlo en D22**: con `contents` el `margin-top: auto` va sobre el precio y el CTA como hijos directos de `.o_wsale_product_information`, no sobre `o_wsale_product_sub`. El CSS cubre los dos casos. Ademas el SCSS **se hace dueno del centrado** del bloque de precio que hoy aporta la edicion a mano de la copia COW 19099 (`text-center` + `flex-column align-items-center`, D22), para que restablecer esa copia a la generica (Plane #68, en produccion) no cambie el layout | T05 | `static/src/scss/payment_method_price.scss`, `specs/website_sale_payment_method_price.md` | CA22 | hecho (CSS cubre ambos casos flex/contents; cual usa el tema de Nokey queda para la verificacion visual del orquestador) |
| **T07** | JS de variantes: espejar el markup nuevo (filas, pill, clases del recuadro), togglear `o_wspmp_discounted` (sin pill ni tachado con recargo). **Restriccion**: `replaceChildren()` **solo** sobre `.o_wspmp_prices`; sobre `o_wspmp_box` y el slot de cuotas, unicamente clases — el `span.oe_price` del core vive ahora dentro del recuadro y lo repinta el `super()` | T02, T05 | `static/src/js/payment_method_price.js` | CA08, CA23 | hecho |
| **T08** | Traduccion `es_AR` de las cadenas nuevas y baja de la vieja | T01 | `i18n/es_AR.po` | CA15 | hecho |
| **T09** | Validacion manual en el navegador: ficha (verificando **cual de los dos layouts** de `product_price` esta activo en Nokey: normal o caja de CTA), grilla, carrito, cambio de variante, 360 px, y que el precio sin impuestos nacionales siga arriba. Ademas: **dar vuelta la regla a `surcharge`** en el sitio de prueba y confirmar que no hay pill ni tachado (CA23); y que el recuadro de la grilla no se desarma con el `display` que el tema le de a `.product_price`. **Se valida sobre la vista del sitio** (`website_id=1`, `lang=es_AR`) —que es la **copia COW** 19099, editada a mano— y nunca sobre la vista generica: ahi es donde se aplica de verdad la herencia (D22) | T01..T08 | — | CA06, CA07, CA08, CA09, CA18, CA19, CA20, CA22, CA23 | hecho lo verificable por `odoo shell` (DOM/orden de la copia COW y de las genericas, sin ParseError); la verificacion visual en navegador la hace el orquestador |
| **T10** | Documentacion del modulo (README + index.html) **y del `__manifest__.py`**: reescribir `summary` (`:5`) y `description` (`:13`), que siguen describiendo el segundo precio de 1.1.0 y el string `with %(method)s` dado de baja en T08; bump del `version` a 1.2.0 y sync de la `Version` de la spec | T09 | `README.md`, `static/description/index.html`, `__manifest__.py`, `specs/website_sale_payment_method_price.md` | — | hecho |

> **CA21 se cierra recien con el puente.** T02/T03 dejan el slot y el contrato; la verificacion
> end-to-end (la linea de cuotas adentro del recuadro) es de la tarea de
> `website_sale_installment_plans_ux` 1.0.0 -> 1.1.0, que **depende de T02/T03**: sin el slot, su
> `position="move"` no tiene destino. Esa tarea tiene que (a) sumar `website_sale_payment_method_price`
> a su `depends` y (b) usar el locator **por `name`** (`//div[@name='wspmp_installments']`), nunca
> `hasclass()` (D21).
