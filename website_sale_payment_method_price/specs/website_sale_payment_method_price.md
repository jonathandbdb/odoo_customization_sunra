...
| **Version** | `1.3.0` (== `version` del `__manifest__.py`, formato `x.x.x`) |
| **Serie Odoo** | `19` (informativa) |
| **Estado** | `implemented` (1.0.2 quedo `verified`; 1.2.0 = rediseno del bloque de precio, implementado y revisado el 11-09-2026; **1.3.0** = segunda maqueta del bloque de precio, la que eligio el cliente el 14-09-2026: un precio por renglon con su pill y la linea "Ahorras $X" en verde. Implementado el 14-09-2026 (T01..T06) y verificado en el navegador por el orquestador sobre la base local con la paleta real de produccion —ficha y tarjeta, con y sin precio de referencia, y cambio de variante—. Queda `implemented` hasta la pasada de @reviewer) |
...
> Segunda maqueta del bloque de precio (1.2.0 -> **1.3.0**), la que eligio el cliente el 14-09-2026.
> Los planes anteriores (T01..T18 del alta, y T01..T10 de 1.2.0) estan cerrados y no se repiten aca.

| Tarea | Descripcion | Depende de | Archivos | Cubre | Estado |
|-------|-------------|-----------|----------|-------|--------|
| **T01** | Datos del ahorro y del pill de referencia: `_get_payment_method_price_vals()` recibe `reference_price` y devuelve `saving_label`; nuevo `_get_payment_method_reference_badge()`; las dos superficies publican `payment_method_reference_badge` (ficha desde `combination_info`, grilla desde `template_price_vals['base_price']`) | — | `models/product_template.py` | CA24, CA25 | hecho |
| **T02** | Ficha: tres `position="replace"` + `$0` que envuelven el precio de lista, el comparativo y el precio actual en su `div.o_wspmp_line` con el pill al lado. Van **despues** del `move` del contenedor (contexto del `t-set`) y el `$0` va **pegado al tag**, sin salto de linea | T01 | `views/website_sale_templates.xml` | CA24, CA25 | hecho |
| **T03** | Grilla: los dos `move` primero (base_price y despues `fw-bold`) y luego los dos envoltorios con su pill | T01 | `views/website_sale_templates.xml` | CA24, CA25 | hecho |
| **T04** | SCSS: `.o_wspmp_line` (precio + pill en el mismo renglon, con el pill en `order: 1` porque `$0` anexa el original al final), contenedor de la ficha en columna, `.o_wspmp_saving` en `#3c7300`, pills en linea (se va el `position: absolute` de la esquina) con `_pm` verde y `_ref` neutro, renglon de referencia escondido cuando no hay que tachar nada, y tamanos reducidos en la tarjeta (D26) | T02, T03 | `static/src/scss/payment_method_price.scss` | CA18, CA24, CA25, CA26 | hecho |
| **T05** | JS de variantes: repintar `saving_label` dentro de cada fila y toglear **los dos** pills por su clase propia (`.o_wspmp_badge_pm` / `.o_wspmp_badge_ref`), no por `.o_wspmp_badge` a secas | T01, T02 | `static/src/js/payment_method_price.js` | CA08, CA24 | hecho |
| **T06** | Traduccion `es_AR` de las cadenas nuevas (`You save %(amount)s`, `-%(percentage)s%%`), bump del manifest a 1.3.0 y sync de la `Version` de la spec; documentacion (README + index.html) | T01..T05 | `i18n/es_AR.po`, `__manifest__.py`, `specs/...md`, `README.md`, `static/description/index.html` | CA15 | hecho |
| **T07** | Verificacion visual en el navegador: ficha y tarjeta a 390 y 1280, con precio comparativo cargado y sin el, y cambio de variante sin errores JS | T01..T06 | — | CA24, CA25, CA26 | hecho (capturas del orquestador sobre la base local con la paleta real) |
